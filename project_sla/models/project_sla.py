...
class ProjectSla(models.Model):
    ...
    target_stage_id = fields.Many2one(
        comodel_name="project.task.type", string="Target Stage", required=True
    )
    ...
    sla_line_ids = fields.One2many(
        "project.sla.line", "sla_id", string="SLA Lines", copy=True
    )
    task_ids = fields.One2many(
        "project.task", string="Tasks", compute="_compute_task_ids"
    )

    @api.depends("sla_line_ids", "sla_line_ids.task_id")
    def _compute_task_ids(self):
        for sla in self:
            sla.task_ids = sla.sla_line_ids.mapped("task_id")
